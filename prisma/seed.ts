...
async function main() {
  ...
  await prisma.apiKey.createMany({
    data: [
      ...
      { merchantId: merchant.id, keyHash: hex64(), name: 'Staging', createdAt: pastDate(60), lastUsedAt: pastDate(8) },
    ],
  })
  ...
  const invPaid = await prisma.invoice.create({
    data: {
      ...
      lineItems: {
        create: [{ description: 'Strategy & Advisory Session (1 hr)', quantity: '1.0000', unitPrice: '400.000000' }],
      },
    },
  })
  ...
  void invUnpaid
  void invOverdue // referenced above to satisfy linter
